...
async function regenerateKey() {
  ...
  try {
    ...
    error.value = `Could not regenerate key: ${e.message}`
  } finally {
...
<template>
  <div class="max-w-2xl space-y-6">
    <div>
      ...
      <p class="text-muted text-sm mt-1">Use this key to authenticate your API requests.</p>
    </div>
  ...
  -d '{
    ...
    <UCard>
      ...
      <ul class="space-y-2 text-sm text-muted">
        <li class="flex items-start gap-2">
          <UIcon name="i-lucide-x-circle" class="text-error mt-0.5 shrink-0" />
      ...
      <template #footer>
        <div class="flex items-center justify-between">
          <p class="text-xs text-muted">If your key is compromised, regenerate it immediately.</p>
          <UButton
            size="sm"
            color="error"
            variant="soft"
    ...
    <UModal v-model:open="showRegenerateConfirm">
      <template #content>
        <UCard>
          <template #header>
            <h3 class="font-semibold text-lg">Regenerate API key?</h3>
          </template>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              <UButton variant="ghost" @click="showRegenerateConfirm = false">Cancel</UButton>
              <UButton color="error" :loading="regenerating" @click="regenerateKey">
                Yes, regenerate
              </UButton>
            </div>
          </template>
